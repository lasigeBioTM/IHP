...
def cross_validation(main_directory):
    ...
    for i in range(len(sets)): #already for the whole process -> 10 times
        ...
        res.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(str(p_stan), str(r_stan), str(p_stan_nr), str(r_stan_nr), str(p_stan_g), str(r_stan_g), str(p_suite), str(r_suite), str(p_suite_nr), str(r_suite_nr), str(p_suite_g), str(r_suite_g)))
