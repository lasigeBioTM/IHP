...
def feat_type(type, min_range, max_range, low_value, top_value):
	...
	for i in range(min_range, max_range):
		val = i
		if type == "non":
			val = low_value + (top_value - low_value)*i*0.04
...
if __name__ == '__main__':
	...
	log = open("src/other/features/log_features.txt", "w")
	good_features = {}
	...
	for line in numerical_features:
		...
		if type == "non":
			best_f_measure, best_val = feat_type(type, 1, 26, float(low_value), float(top_value))
