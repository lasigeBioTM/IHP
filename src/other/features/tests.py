...
def brown_clusters(crf):
    ...
    num_clusters_values = [50, 75, 95, 100, 125, 150, 250, 500]
    num_col_values = [50, 100, 150, 250, 500, 750]
    min_occur_values = [1,2,3,5, 10]
    ...
    for clu in num_clusters_values:
        for col in num_col_values:
            for occ in min_occur_values:
                i += 1
                os.system("(cd bin/brown-cluster/; ./wcluster --text ../temp/full_corpus_separated.txt --c {} --min-occur {} --ncollocs {} --output_dir ../temp/clusters)".format(str(clu), str(occ), str(col)))
                ...
                final_results.write(str(f_measure) + "\t" + str(clu) + "\t" + str(col) + "\t" + str(occ) + "\n")

    final_results.close()

def feature_selection():
    os.system("python src/other/bool_feature_selection.py")
    os.system("python src/other/numerical_feature_selection.py")
